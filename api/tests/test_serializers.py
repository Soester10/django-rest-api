...
from api.serializers import UserRegisterSerializer, UserLoginSerializer, TagSerializer, DataSerializer

class UserSerializerTestCase(TestCase):
    def test_user_register_serializer_create(self):
        serializer = UserRegisterSerializer(data={'email': 'test@example.com', 'password': 'testpassword', 'username': 'testuser'})
        self.assertTrue(serializer.is_valid())
        user = serializer.save()
        self.assertIsInstance(user, get_user_model())

    def test_user_login_serializer_validate(self):
        Custom_User.objects.create_user(username='testuser', email='test@example.com', password='testpassword')
        serializer = UserLoginSerializer(data={'username': 'testuser', 'password': 'testpassword'})
        self.assertTrue(serializer.is_valid())
